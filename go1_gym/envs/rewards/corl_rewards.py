...
class CoRLRewards:
    ...
    def _reward_FL_foot_height_tracking(self):
        # agent feet heights
        reference_heights = 0
        FL_foot_height = (self.env.foot_positions[:, 0, 2]).view(
            self.env.num_envs, -1) - reference_heights
    ...
    def _reward_FR_foot_height_tracking(self):
        # agent feet heights
        reference_heights = 0
        FR_foot_height = (self.env.foot_positions[:, 1, 2]).view(
            self.env.num_envs, -1) - reference_heights
    ...
    def _reward_RL_foot_height_tracking(self):
        # agent feet heights
        reference_heights = 0
        RL_foot_height = (self.env.foot_positions[:, 2, 2]).view(
            self.env.num_envs, -1) - reference_heights

        rew_feet_clearance = torch.square(torch.sum(RL_foot_height, dim=1))
        return rew_feet_clearance

    def _reward_RR_foot_height_tracking(self):
        # agent feet heights
        reference_heights = 0
        RR_foot_height = (self.env.foot_positions[:, 3, 2]).view(
            self.env.num_envs, -1) - reference_heights

        rew_feet_clearance = torch.square(torch.sum(RR_foot_height, dim=1))
        return rew_feet_clearance
