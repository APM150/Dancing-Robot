...
class RCControllerProfile(CommandProfile):
    def __init__(self, dt, state_estimator, x_scale=1.0, y_scale=1.0, yaw_scale=1.0, probe_vel_multiplier=1.0):
        ...
        for i, file_name in enumerate(glob.glob("../../motion_folder/*.pkl")):
            ...
            # normalize to [0, 0.2]
            joint3d[:, feet_idx[0], 2] = (left_feet_heights - min_height) / (max_height - min_height) * 2.5
            joint3d[:, feet_idx[1], 2] = (right_feet_heights - min_height) / (max_height - min_height) * 2.5
            all_joint3d[i] = joint3d
        self.all_joint3d = all_joint3d

        joint3d = all_joint3d[2]
        feet_idx = [7, 8]
        self.FL_height = joint3d[:, feet_idx[0], 2]
        self.FR_height = joint3d[:, feet_idx[1], 2]
    ...
    def get_command(self, t, probe=False):
        ...
        try:
            command[15] = self.FL_height[int(np.round(t*30))]
            command[16] = self.FR_height[int(np.round(t*30))]
